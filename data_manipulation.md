...
select helpers – want to use functions that help me select columns that
i know i want to keep based on some criteria
...
intended to remove or keep rows that we are interested in retaining for
our analyses - needs to be given a condition
...
instead, USE PIPE OPERATOR INSTEAD (shift, command, m) - easier to read
and take a look at
